...
bool ArduinoCalc::calculate(const String& input, double& result, String& errMsg, String& outStr) {
    ...
    if (isMultiBase) {
        ...
        if (finalVal.type == VT_MPI) {
            ...
            outStr.concat(sign); outStr.concat("0o"); outStr.concat(s_oct); outStr.concat("\n\n");
        } else {
            long intVal = lround(finalVal.d_val);
            unsigned long uVal = (intVal < 0) ? (unsigned long)(-intVal) : (unsigned long)intVal;
            const char * sign = (intVal < 0) ? "-" : "";
            String decStr = String(uVal);
            String hexStr = String(uVal, HEX); hexStr.toUpperCase();
            String binStr = String(uVal, BIN);
            String octStr = String(uVal, OCT);

            outStr =      sign ;                      outStr.concat(decStr); outStr.concat(" ");
            outStr.concat(sign); outStr.concat("0x"); outStr.concat(hexStr); outStr.concat(" ");
            outStr.concat(sign); outStr.concat("0b"); outStr.concat(binStr); outStr.concat(" ");
            outStr.concat(sign); outStr.concat("0o"); outStr.concat(octStr); outStr.concat(" ");
        }
    } else {
        outStr = "";
        if (finalVal.type == VT_MPI) {
